...
PATREON_REDIRECT = os.getenv('PATREON_REDIRECT_URI')


def get_patreon_subscribe_url():
    client_id = PATREON_CLIENT_ID
    redirect_uri = "&redirect_uri=" + PATREON_REDIRECT
    v2Params = "&scope=identity%20identity[email]"
    url = f'https://www.patreon.com/oauth2/become-patron?response_type=code$&min_cents=500&client_id={client_id}${redirect_uri}${v2Params}'
    return f'''<a className="patreon-button link-button" data-patreon-widget-type="become-patron-button" href={url} rel="noreferrer" target="_blank">
      /* 
      <svg
        id="patreon-logo"
